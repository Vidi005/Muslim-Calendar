...
const MoonCrescentMapContent = ({ t, state, selectedLanguage, selectedTimeZone, selectHijriMonth, selectedMoonVisibilityCriteria, restoreToDefault }) => (
  <article className="prayer-times-content flex flex-col grow bg-green-100 dark:bg-gray-700 overflow-y-auto duration-200">
    ...
    <div className="grid grid-flow-row items-stretch mb-4 grow md:flex-none">
      {state.areMoonVisibilityCriteriaMapsLoading
        ...
        : (
          <>
            <h4 className="m-1 text-sm sm:text-base md:text-lg text-center text-green-700 dark:text-gray-200 duration-200">{t('conjunction')} {state.moonCrescentVisibilities.at(0)?.conjunction?.toLocaleString(selectedLanguage || "en", { weekday: "long", day: "2-digit", month: "2-digit", year: "numeric", hourCycle: "h23", hour: "2-digit", minute: "2-digit", second: "2-digit", timeZoneName: "short", timeZone: selectedTimeZone }).replace(/Minggu/g, 'Ahad').replace(/\./g, ':')}</h4>
