...
const PrayerTimesVisualization = ({ t, selectedLanguage, inputDate, inputTime, formattedDateTime, timeZone, seletedDhuhaMethod, dhuhaSunAltitude, sunAltitude, currentPrayerTimes, areSunInfosLoading, sunInfos }) => {
  ...
  const duhrPosition = ((90 - parseFloat(sunInfos[sunInfos.length - 2])) / 90) * 50
      ...
      <div className="flex flex-wrap justify-center w-full">
        ...
        <p className="text-sm md:text-base xl:text-lg">{t('zenith')}</p>
          <div className="relative flex-auto flex flex-col items-center justify-center border border-green-900 dark:border-white w-full h-max text-xs lg:text-sm 2xl:text-base duration-300 overflow-hidden">
            ...
            <span className="absolute w-[4%] border border-yellow-700 bg-yellow-400 rounded-full -translate-x-1/2 translate-y-1/2 duration-300" style={{ bottom: `${ySunPosition}%`, left: `${xSunPosition}%` }}>
              <img className="w-full object-contain object-center" src={`${import.meta.env.BASE_URL}images/circle-icon.png`} alt="Sun" />
            </span>
            <span className={`${parseFloat(sunInfos[3]) > 90 && parseFloat(sunInfos[3]) <= 180 || (parseFloat(sunInfos[3]) > 270 && parseFloat(sunInfos[3]) <= 360) ? "-translate-x-full" : "translate-x-1/2"} absolute translate-y-1/2 px-0.5 text-xs md:text-sm xl:text-base text-center text-yellow-600 dark:text-yellow-300 duration-300`} style={{ bottom: `${ySunPosition}%`, left: `${xSunPosition}%` }}>{t('objects.0')}</span>
            {formattedDateTime > sunInfos[sunInfos.length - 3] && (
              <React.Fragment>
                ...
                <span className="absolute mt-1 w-full text-center duration-200" style={{ top: `${duhrPosition}%` }}><b>{t('prayer_names.4')} {t('at')} {currentPrayerTimes?.at(4)?.toLocaleTimeString(selectedLanguage || 'en', { hourCycle: "h24", hour: '2-digit', minute: '2-digit', second: '2-digit', timeZone: timeZone }).replace(/\./gm, ':') || ''} (+{sunInfos[sunInfos.length - 2]})</b></span>
              </React.Fragment>
            )}
            {formattedDateTime < sunInfos[sunInfos.length - 3] && dhuhaPosition > 0 && (
              <React.Fragment>
            ...
            {formattedDateTime > sunInfos[sunInfos.length - 3] && ishaPosition > 0 && (
              <React.Fragment>
            ...
            <span className="absolute horizon translate-y-1/2 border-2 border-solid border-green-900 dark:border-white w-full bg-green-900 dark:bg-white"></span>
            <span className="absolute w-full h-1/2 bottom-0 bg-green-900/20 dark:bg-white/25"></span>
            <span className="absolute w-full bottom-1/2 text-center text-green-500 dark:text-gray-300">{formattedDateTime > sunInfos[sunInfos.length - 3] ? t('horizons.1') : t('horizons.0')}</span>
            ...
            <span className="civil-twilight absolute w-full px-2 bottom-[46.67%] text-right text-green-600 dark:text-gray-300">{formattedDateTime > sunInfos[sunInfos.length - 3] ? t('civil_twilight.1') : t('civil_twilight.0')} (-6°)</span>
            ...
            <span className="absolute w-full px-2 bottom-[43.33%] text-right text-green-600 dark:text-gray-300">{formattedDateTime > sunInfos[sunInfos.length - 3] ? t('nautical_twilight.1') : t('nautical_twilight.0')} (-12°)</span>
            ...
            <span className="absolute w-full px-2 bottom-[40%] text-right text-green-600 dark:text-gray-300">{formattedDateTime > sunInfos[sunInfos.length - 3] ? t('astronomical_twilight.1') : t('astronomical_twilight.0')} (-18°)</span>
          </div>
