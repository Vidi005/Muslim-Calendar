...
const PrayerTimesVisualization = ({ t, selectedLanguage, inputDate, inputTime, formattedDateTime, timeZone, seletedDhuhaMethod, dhuhaSunAltitude, sunAltitude, currentPrayerTimes, areSunInfosLoading, sunInfos }) => {
  ...
  const duhrPosition = ((90 - parseFloat(sunInfos[sunInfos.length - 3])) / 90) * 50
  const ashrPosition = ((90 - parseFloat(sunInfos[sunInfos.length - 2])) / 90) * 50
      ...
      <div className="flex flex-wrap justify-center w-full">
        ...
        <p className="text-sm md:text-base xl:text-lg">{t('zenith')} (90°)</p>
          <div className="relative flex-auto flex flex-col items-center justify-center border border-green-900 dark:border-white w-full h-max text-xs lg:text-sm 2xl:text-base duration-300 overflow-hidden">
            ...
            <span className="absolute w-[3%] border border-yellow-700 bg-yellow-400 rounded-full -translate-x-1/2 translate-y-1/2 duration-300" style={{ bottom: `${ySunPosition}%`, left: `${xSunPosition}%` }}>
            ...
            {formattedDateTime > sunInfos[sunInfos.length - 4] && (
              <React.Fragment>
                <span className="absolute m-0.5 w-full duration-200" style={{ top: `${ashrPosition}%` }}><b>{t('prayer_names.5')} {t('at')} {currentPrayerTimes?.at(5)?.toLocaleTimeString(selectedLanguage || 'en', { hourCycle: "h24",  hour: '2-digit', minute: '2-digit', second: '2-digit', timeZone: timeZone }).replace(/\./gm, ':') || ''} (+{sunInfos[sunInfos.length - 2]})</b></span>
            ...
            {formattedDateTime > sunInfos[sunInfos.length - 4] && ishaPosition > 0 && (
              <React.Fragment>
            ...
            <span className="civil-twilight absolute w-full px-2 bottom-[46.67%] text-right text-green-600 dark:text-gray-300">{formattedDateTime > sunInfos[sunInfos.length - 4] ? t('civil_twilight.1') : t('civil_twilight.0')} (-6°)</span>
            ...
            <span className="absolute w-full px-2 bottom-[43.33%] text-right text-green-600 dark:text-gray-300">{formattedDateTime > sunInfos[sunInfos.length - 4] ? t('nautical_twilight.1') : t('nautical_twilight.0')} (-12°)</span>
            ...
            <span className="absolute w-full px-2 bottom-[40%] text-right text-green-600 dark:text-gray-300">{formattedDateTime > sunInfos[sunInfos.length - 4] ? t('astronomical_twilight.1') : t('astronomical_twilight.0')} (-18°)</span>
          </div>
