            ...
            : (
              <>
                ...
                <div className="prayer-times-list-container-print hidden flex-col items-center flex-nowrap h-[25.9cm] w-[19.5cm] m-[1cm]">
                  <h2 className={`${monthType === 0 ? "" : "leading-snug"} font-serif text-center text-green-900`}>{schedule} {selectedMonth.replace(/-/gm, ' ')}</h2>
                  <h3 className={`${monthType === 0 ? "" : "leading-snug"} font-serif text-center text-green-900`}>{selectedLocation?.city}, {selectedLocation?.admin_name}, {selectedLocation?.country}</h3>
                  <h4 className={`${monthType === 0 ? "" : "leading-snug"} text-center text-black`}>{t('coordinates')} ({t('latitude')} {latitude.toFixed(7)}, {t('longitude')} {longitude.toFixed(7)}, {t('elevation')} {elevation.toFixed(1)} m)</h4>
                  <h5 className={`${monthType === 0 ? "" : "leading-snug"} mb-1 text-center text-black`}>{t('convention')} {t(`conventions.${selectedConvention}.method`)}, {t('timezone')} {selectedTimeZone}, {t('school')} {t(`mazhab.${selectedAshrTime}`)}</h5>
                ...
                <div className="prayer-times-list-container-download hidden">
                  <h2 style={{ lineHeight: monthType === 0 ? "1.5pt" : "1.25pt", textAlign: "center", fontFamily: "'Times New Roman', 'Serif'", fontSize: "16pt", color: "#14532d" }}>{schedule} {selectedMonth.replace(/-/gm, ' ')}</h2>
                  <h3 style={{ lineHeight: monthType === 0 ? "1.5pt" : "1.25pt", textAlign: "center", fontFamily: "'Times New Roman', 'Serif'", fontSize: "14pt", color: "#14532d" }}>{selectedLocation?.city}, {selectedLocation?.admin_name}, {selectedLocation?.country}</h3>
                  <h4 style={{ lineHeight: monthType === 0 ? "1.5pt" : "1.25pt", textAlign: "center", fontFamily: "'Helvetica', 'Arial'", fontSize: "12pt", color: "black" }}>{t('coordinates')} ({t('latitude')} {latitude.toFixed(7)}, {t('longitude')} {longitude.toFixed(7)}, {t('elevation')} {elevation.toFixed(1)} m)</h4>
                  <h5 style={{ marginBottom: "1pt", lineHeight: monthType === 0 ? "1.5pt" : "1.25pt", textAlign: "center", fontFamily: "'Helvetica', 'Arial'", fontSize: "11pt", color: "black" }}>{t('convention')} {t(`conventions.${selectedConvention}.method`)}, {t('timezone')} {selectedTimeZone}, {t('school')} {t(`mazhab.${selectedAshrTime}`)}</h5>
