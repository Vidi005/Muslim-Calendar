...
const CurrentPrayerTimes = () => (
  <HomePageConsumer>
    {({ t, state }) => {
      ...
      let prayerNames = 0
      if (state.monthsInSetYear.length > 0) {
        hijriMonthNumber = getHijriDate(state.formattedDateTime, state.monthsInSetYear).islamicMonth
        prayerNames = hijriMonthNumber === 9
      ...
      return (
        <section className="flex-1 flex flex-col items-center w-full md:w-1/2 p-2 md:px-4 2xl:px-8 text-green-700 dark:text-gray-200 duration-200 animate__animated animate__fadeInLeft">
          ...
          {state.arePrayerTimesLoading || state.monthsInSetYear.length < 1
            ? (
