...
const LunarEclipseMap = ({ t, selectedLanguage, selectedTimeZone, lunarEclipseData, isUpcomingLunarEclipseMapLoading, upcomingLunarEclipseResult }) => (
  <div className="upcoming-lunar-eclipse-container flex flex-wrap md:flex-nowrap items-center md:items-start w-full md:space-x-4 xl:space-x-8 animate__animated animate__fadeInUp">
    {isUpcomingLunarEclipseMapLoading
      ? (
        <div className="flex basis-full md:basis-3/5 items-center justify-center space-x-2 p-2 md:p-4">
          ...
          <span className="text-center text-green-700 dark:text-gray-200 text-base md:text-lg lg:text-xl">{t('upcoming_lunar_eclipse_map_loading')}</span>
        </div>
      )
      : (
        <div className="flex flex-col basis-full md:basis-3/5 items-center justify-center space-x-2 animate__animated animate__fadeInLeft">
          ...
          <div className={`grid w-full p-1 md:p-2 gap-1 md:gap-2 items-stretch ${en.eclipse_types[1]?.kinds?.length > 2 ? "grid-cols-3" : "grid-cols-2"}`}>
            {en.eclipse_types[1]?.kinds?.map((kind, index) => (
              <div key={index} className="flex flex-nowrap items-baseline space-x-1 md:space-x-1.5 text-xs md:text-sm xl:text-base">
                ...
                <span className="text-black dark:text-white">{t(`eclipse_types.1.kinds.${index}.name`)}</span>
              </div>
            ))}
          </div>
        </div>
      )}
    <div className="flex basis-full md:basis-2/5 items-center justify-center space-x-2 animate__animated animate__fadeInRight">
      <LunarEclipseItem
        t={t}
        selectedLanguage={selectedLanguage}
        selectedTimeZone={selectedTimeZone}
        lunarEclipse={lunarEclipseData} />
    </div>
  </div>
)

export default LunarEclipseMap