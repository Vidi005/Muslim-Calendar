...
const GlobalSolarEclipseItem = ({ t, selectedLanguage, selectedTimeZone, globalSolarEclipse }) => {
  let globalSolarEclipseDate = ''
  let solarEclipsePeakTime = ''
  try {
    globalSolarEclipseDate = globalSolarEclipse?.peak?.toLocaleDateString(selectedLanguage || 'en', { calendar: 'gregory', weekday: 'long', day: '2-digit', month: 'long', year: 'numeric', timeZone: selectedTimeZone }).replace(/Minggu/g, 'Ahad').replace(/Jumat/g, 'Jum\'at')
    solarEclipsePeakTime = `${globalSolarEclipse?.peak?.toLocaleTimeString(selectedLanguage || 'en', { timeZone: selectedTimeZone, timeZoneName: 'short', hour12: false, hourCycle: 'h23', hour: '2-digit', minute: '2-digit', second: '2-digit' }).replace(/\./g, ':')}`
  } catch (error) {
    globalSolarEclipseDate = t('unsupported_time_format')
  ...
  return (
    <div className="w-full bg-green-50 dark:bg-black p-2 md:px-4 text-base md:text-sm lg:text-base rounded-md md:rounded-lg shadow lg:shadow-md dark:shadow-white/50 overflow-hidden duration-200">
      <h4 className="mb-1 text-center text-base md:text-sm lg:text-base text-green-900 dark:text-white font-bold">{globalSolarEclipseDate}</h4>
