        ...
        {state.isMoonCrescentMapLoading
          ...
          : (
            <React.Fragment>
              ...
              <h5 className="mb-1 text-center text-green-700 dark:text-gray-200 duration-200"><i>{t('conjunction')}</i>: {state.moonCrescentVisibility.conjunction?.toLocaleString(state.selectedLanguage || "en", { calendar: "gregory", day: "2-digit", month: "2-digit", year: "numeric", hourCycle: "h23", hour: "2-digit", minute: "2-digit", timeZoneName: "short", timeZone: state.selectedTimeZone })?.replace(/\./g, ':')}, {t('observation_date')} {state.moonCrescentVisibility.observationDate?.toLocaleString(state.selectedLanguage || "en", { calendar: "gregory", day: "2-digit", month: "2-digit", year: "numeric" })}</h5>
              <div className="relative w-full border sm:border-2 md:border-4 border-green-900 dark:border-white rounded duration-200 overflow-hidden">
                ...
                {state.moonCrescentVisibility.zoneCoordinates[0]?.fajrAtWellington && (
                  <span className="absolute origin-bottom-right bottom-0 right-0 p-1 md:p-2 text-[8px] sm:text-xs 2xl:text-sm text-orange-100">{t('fajr_in_new_zealand')}: {state.moonCrescentVisibility.zoneCoordinates[0].fajrAtWellington?.toLocaleString(state.selectedLanguage || "en", { calendar: "gregory", day: "2-digit", month: "2-digit", year: "numeric", hourCycle: "h23", hour: "2-digit", minute: "2-digit", timeZoneName: "short", timeZone: state.selectedTimeZone })?.replace(/\./g, ':')} ({state.moonCrescentVisibility.zoneCoordinates[0].fajrAtWellington?.toLocaleString(state.selectedLanguage || "en", { calendar: "gregory", day: "2-digit", month: "2-digit", year: "numeric", hourCycle: "h23", hour: "2-digit", minute: "2-digit", timeZoneName: "short", timeZone: 'UTC' })?.replace(/\./g, ':')})</span>
                )}
              </div>
