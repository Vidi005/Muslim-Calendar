...
const AboutContent = ({ t }) => (
  <article className="about-content grow p-4 lg:p-8 font-serif text-green-900 dark:text-green-100 text-center duration-200 overflow-y-auto animate__animated animate__fadeInUpBig">
    <section className="mb-4">
      <h2 className="text-center text-green-900 dark:text-white">{t('app_name')}</h2>
      <h5 className="my-4 text-center text-gray-700 dark:text-gray-200">{t('version')}</h5>
      <p className="text-base 2xl:text-lg text-justify text-black dark:text-gray-100">{t('about')}</p>
    </section>
    <section className="my-4 text-left">
      ...
      <ul className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-0.5 md:gap-1 list-inside list-disc text-base 2xl:text-lg text-blue-500 dark:text-blue-400 underline">
