...
const AboutContent = ({ t }) => (
  <article className="about-content grow p-4 lg:p-8 text-green-900 dark:text-green-100 text-center duration-200 overflow-y-auto animate__animated animate__fadeInUpBig">
    ...
    <section className="my-4 text-left">
      ...
      <ul className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-0.5 md:gap-1 list-inside list-disc text-sm lg:text-base 2xl:text-lg text-blue-600 dark:text-blue-400 underline">
        {en.resource_items.map((_item, index) => (
          <li key={index} className="w-full sm:w-auto p-1 hover:text-blue-800 dark:hover:text-blue-300 active:text-purple-700 dark:active:text-purple-500"><a href={t(`resource_items.${index}.url`)} target="_blank" rel="noopener noreferrer">{t(`resource_items.${index}.name`)}</a></li>
        ))}
      </ul>
    </section>
    <section className="my-4 text-left">
      ...
      <ul className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-0.5 md:gap-1 list-inside list-disc text-sm lg:text-base 2xl:text-lg text-blue-600 dark:text-blue-400 underline">
        {en.reference_items.map((_item, index) => (
          <li key={index} className="w-full sm:w-auto p-1 hover:text-blue-800 dark:hover:text-blue-300 active:text-purple-700 dark:active:text-purple-500"><a href={t(`reference_items.${index}.url`)} target="_blank" rel="noopener noreferrer">{t(`reference_items.${index}.name`)}</a></li>
        ))}
      </ul>
    </section>
  </article>
