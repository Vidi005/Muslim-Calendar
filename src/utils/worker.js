...
const CACHE_NAME = 'app-cache-v6.2' // Update the version when deploying new builds to clear old caches
...
self.onmessage = event => {
  const { type, months, setMonths, gregorianDate, formattedDateTime, cityData, query, latitude, longitude, elevation, criteria, sunAltitude, formula, lang, innerHTML, timeZone, calculationMethod, ashrTime, zawal, ihtiyath, corrections, dhuhaMethod, inputSunAlt, inputMins, observationDate, moonVisibilityCriteria, elongationType, altitudeType, observationTime, correctedRefraction, steps, globalSolarEclipseDate, localSolarEclipseDate, lunarEclipseDate } = event.data
